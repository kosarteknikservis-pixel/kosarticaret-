    ...
    <div class="admin-settings admin-settings-shell">
        ...
        <form method="post"
            ...
            <div id="settings-panel-general" class="admin-settings-panel {{ $activeTab !== 'general' ? 'hidden' : '' }}" role="tabpanel" aria-labelledby="settings-tab-general">
                ...
                <div class="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <h4 class="text-sm font-bold text-slate-900">Form spam koruması (Google reCAPTCHA v2)</h4>
                    <p class="mt-2 text-sm text-slate-600">
                        İletişim, ürün yorumu ve teklif formlarında &ldquo;Ben robot değilim&rdquo; kutusu gösterilir.
                        Anahtarlar <a href="https://www.google.com/recaptcha/admin/create" class="text-teal-700 font-semibold hover:text-teal-900" target="_blank" rel="noopener">Google reCAPTCHA Admin</a> üzerinden alınır.
                        Tür: <strong>reCAPTCHA v2</strong> → <strong>&ldquo;Ben robot değilim&rdquo; onay kutusu</strong>.
                        Alan adı: <strong>kosarticaret.com</strong>.
                    </p>
                    @if(\App\Support\ContactFormSpamGuard::recaptchaMisconfigured())
                        <p class="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
                            Girilen anahtarlar geçerli görünmüyor. Site key ve secret key aynı reCAPTCHA v2 kaydından olmalıdır.
                        </p>
                    @endif
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        <div>
                            <label class="admin-label">Site key (public)</label>
                            <input name="recaptcha_site_key" value="{{ $values['recaptcha_site_key'] ?? '' }}" class="admin-input font-mono text-sm" placeholder="6Lc...">
                        </div>
                        <div>
                            <label class="admin-label">Secret key</label>
                            <input name="recaptcha_secret_key" value="{{ $values['recaptcha_secret_key'] ?? '' }}" class="admin-input font-mono text-sm" placeholder="6Lc..." autocomplete="off">
                        </div>
                    </div>
                </div>
